...
import { ArrowLeft, Trash2, Mic, Lock, ShieldCheck, Save, Share2, Book, Clock } from 'lucide-react';
import { SpeechRecognition } from '@capacitor-community/speech-recognition';
import { App as CapApp } from '@capacitor/app';
import CryptoJS from 'crypto-js';

export default function SecureJournal({ goHome }) {
  const [view, setView] = useState('lock'); // 'lock', 'list', 'editor'
  ...
  useEffect(() => {
    const listener = CapApp.addListener('backButton', () => {
      if (view === 'lock') { if (goHome) goHome(); } 
      else if (view === 'editor') { setView('list'); } 
      else if (view === 'list') { setView('lock'); }
    });
  ...
  const handlePinTap = (num) => {
    ...
    if (updated.length <= 4) {
      setPasscode(updated);
      if (updated.length === 4) {
        if (updated === CORRECT_PIN) { setView('list'); setPasscode(''); } 
  ...
  const exportAES = () => {
    if (!entryText.trim()) return;
    // Scramble the entry using AES encryption, keyed by her exact PIN
    const ciphertext = CryptoJS.AES.encrypt(entryText, CORRECT_PIN).toString();
    const body = `AES ENCRYPTED JOURNAL ENTRY:\n\n${ciphertext}\n\n(Decrypt using your PIN)`;
    window.location.href = `mailto:?subject=Secure Encrypted Journal&body=${encodeURIComponent(body)}`;
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center', alignItems: 'center', paddingBottom: '24px' }}>
        ...
        <div style={{ backgroundColor: 'var(--surface)', border: '3px solid var(--accent)', borderRadius: '24px', padding: '32px', width: '100%', maxWidth: '360px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
            ...
            <button onClick={() => { if (passcode === CORRECT_PIN) { setView('list'); setPasscode(''); } }} style={{ height: '70px', backgroundColor: 'var(--accent)', color: '#000', borderRadius: '16px', border: 'none', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><ShieldCheck size={32} /></button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', paddingBottom: '24px', paddingTop: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <h2 style={{ margin: 0, color: 'var(--accent)' }}>My Archive</h2>
        </div>
        
        <button onClick={openNewEntry} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', backgroundColor: 'var(--accent)', color: '#000', padding: '24px', borderRadius: '20px', fontSize: '24px', fontWeight: 'bold', border: 'none', marginBottom: '24px' }}>
          <Book size={28} /> Write New Entry
        </button>

        <div style={{ flexGrow: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {entries.length === 0 ? (
            <p style={{ textAlign: 'center', color: '#888', fontSize: '20px', marginTop: '40px' }}>No entries yet.</p>
          ) : (
            entries.map(entry => {
              const locked = (Date.now() - entry.timestamp > 86400000);
              return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', paddingBottom: '24px', paddingTop: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <button onClick={() => { if(!isPast24Hours) saveCurrentEntry(); setView('list'); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#333', padding: '14px 20px', borderRadius: '16px', color: '#FFF', fontSize: '22px', fontWeight: 'bold', border: 'none' }}>
          <ArrowLeft size={24} /> Back
        </button>
      ...
      <div style={{ border: '2px dashed var(--accent)', borderRadius: '16px', padding: '16px', textAlign: 'center', marginBottom: '20px' }}>
        <p style={{ color: 'var(--accent)', fontSize: '20px', fontStyle: 'italic', margin: 0 }}>
          {isPast24Hours ? "This entry is securely locked for safekeeping." : "\"Your words matter. Take your time.\""}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {!isPast24Hours && (
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button onClick={() => { saveCurrentEntry(); alert("Saved securely!"); }} style={{ flex: 1, backgroundColor: '#2E7D32', border: 'none', borderRadius: '16px', color: '#FFF', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <Save size={28} />
            </button>
        ...
        <button onClick={exportAES} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', backgroundColor: '#1E3A8A', border: 'none', borderRadius: '16px', padding: '20px', color: '#FFF', fontSize: '20px', fontWeight: 'bold' }}>
          <Share2 size={28} /> Export Encrypted (AES)
        </button>
      </div>
    </div>
  );
