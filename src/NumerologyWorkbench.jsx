...
export default function NumerologyWorkbench({ goHome }) {
  ...
  const switchView = (newView) => { setView(newView); setNotes(''); };
  ...
  const calculateAll = (dateString, fullNameString = '') => {
    ...
    return { lifePath, birthDayNum, attitudeNum, personalYear, personalMonth, personalDay, universalYear, universalMonth, universalDay, destinyNum, soulUrgeNum, personalityNum };
  };
  ...
  const saveReadingToHistory = (type) => {
    ...
    if (!nums) return;
    const newEntry = { id: Date.now(), type, name, birthdate: date, numbers: nums, notes, savedAt: new Date().toLocaleDateString() };
  ...
  const renderNumRow = (title, num, highlight = false) => (
  ...
  const renderReadingCard = (nums, name, onSave) => {
    const data = NUMEROLOGY_DATA[nums.lifePath] || { meaning: "A beautiful and unique path awaits you.", planet: "Unknown", sign: "Unknown" };
    return (
      <div style={{ paddingBottom: '24px' }}>
        <h2 style={{ color: '#FFF', fontSize: '28px', textAlign: 'center', margin: '0 0 24px 0' }}>Today's Energies for {name}</h2>
        <div style={{ border: '2px solid #FF9500', borderRadius: '24px', padding: '24px', backgroundColor: '#111' }}>
          ...
          {renderNumRow("Birth Day Number", nums.birthDayNum)}
          {renderNumRow("Attitude Number", nums.attitudeNum)}
          {nums.destinyNum > 0 && (
            <>
              <div style={{ borderTop: '2px dashed #444', margin: '24px 0' }} />
              {renderNumRow("Destiny Number", nums.destinyNum)}
              {renderNumRow("Soul Urge Number", nums.soulUrgeNum)}
              {renderNumRow("Personality Number", nums.personalityNum)}
            </>
          )}
          <div style={{ borderTop: '2px dashed #444', margin: '24px 0' }} />
          {renderNumRow("Universal Year", nums.universalYear)}
          {renderNumRow("Universal Month", nums.universalMonth)}
          {renderNumRow("Universal Day", nums.universalDay)}
          {renderNumRow("Personal Year", nums.personalYear)}
          {renderNumRow("Personal Month", nums.personalMonth)}
          {renderNumRow("Personal Day", nums.personalDay)}
        </div>

        <h3 style={{ margin: '24px 0 12px 0', fontSize: '24px', color: '#FFF' }}>Notes on this reading:</h3>
        <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Tap here to write down your thoughts..." style={{ flexGrow: 1, minHeight: '120px', backgroundColor: '#222', color: '#FFF', fontSize: '20px', padding: '16px', borderRadius: '16px', border: '2px solid #444', resize: 'none', outline: 'none' }} />
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', paddingTop: '12px', paddingBottom: '24px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '32px' }}>
        <button onClick={() => switchView('mine')} style={{ flex: 1, backgroundColor: view === 'mine' ? '#FF9500' : '#222', color: view === 'mine' ? '#000' : '#FFF', padding: '16px', borderRadius: '16px', fontSize: '20px', fontWeight: 'bold', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}><User size={24} /> My Daily<br/>Numbers</button>
        <button onClick={() => switchView('other')} style={{ flex: 1, backgroundColor: view === 'other' ? '#FF9500' : '#222', color: view === 'other' ? '#000' : '#FFF', padding: '16px', borderRadius: '16px', fontSize: '20px', fontWeight: 'bold', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}><Users size={24} /> Read for<br/>Others</button>
        <button onClick={() => switchView('history')} style={{ flex: 1, backgroundColor: view === 'history' ? '#FF9500' : '#222', color: view === 'history' ? '#000' : '#FFF', padding: '16px', borderRadius: '16px', fontSize: '20px', fontWeight: 'bold', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}><HistoryIcon size={24} /> History</button>
      </div>
        ...
        <>
          {(!myBday || isEditingMine) ? (
            <div style={{ backgroundColor: '#111', padding: '24px', borderRadius: '20px', border: '2px solid #444' }}>
              ...
              <input type="date" value={myBday} onChange={(e) => setMyBday(e.target.value)} style={{ width: '100%', backgroundColor: '#222', color: '#FFF', fontSize: '28px', padding: '20px', borderRadius: '16px', border: '2px solid #FF9500', outline: 'none', marginBottom: '20px', boxSizing: 'border-box' }} />
              <h2 style={{ color: '#FFF', margin: '0 0 16px 0', fontSize: '24px' }}>Full Birth Name (Optional):</h2>
              <input type="text" value={myFullName} onChange={(e) => setMyFullName(e.target.value)} placeholder="For Destiny numbers..." style={{ width: '100%', backgroundColor: '#222', color: '#FFF', fontSize: '24px', padding: '20px', borderRadius: '16px', border: '2px solid #555', outline: 'none', marginBottom: '24px', boxSizing: 'border-box' }} />
              <button onClick={saveMyInfo} style={{ width: '100%', backgroundColor: '#FF9500', color: '#000', fontSize: '24px', fontWeight: 'bold', border: 'none', borderRadius: '16px', padding: '20px' }}>Lock in My Blueprint</button>
            </div>
          ) : (
            <div style={{ position: 'relative' }}>
              <button onClick={() => setIsEditingMine(true)} style={{ position: 'absolute', top: '0px', right: '16px', backgroundColor: '#333', border: 'none', color: '#FFF', padding: '12px', borderRadius: '12px', zIndex: 10 }}><Edit3 size={24} /></button>
              {renderReadingCard(calculateAll(myBday, myFullName), "You", () => saveReadingToHistory('mine'))}
            </div>
          )}
        </>
      )}
      ...
      {view === 'other' && (
        <>
          <div style={{ backgroundColor: '#111', padding: '24px', borderRadius: '20px', border: '2px solid #444', marginBottom: '24px' }}>
            ...
            <input type="text" value={otherName} onChange={(e) => setOtherName(e.target.value)} placeholder="e.g. Will" style={{ width: '100%', backgroundColor: '#222', color: '#FFF', fontSize: '24px', padding: '16px', borderRadius: '12px', border: '2px solid #555', outline: 'none', marginBottom: '20px', boxSizing: 'border-box' }} />
            <h2 style={{ color: '#FFF', margin: '0 0 12px 0', fontSize: '22px' }}>Birthdate:</h2>
            <input type="date" value={otherBday} onChange={(e) => setOtherBday(e.target.value)} style={{ width: '100%', backgroundColor: '#222', color: '#FFF', fontSize: '24px', padding: '16px', borderRadius: '12px', border: '2px solid #FF9500', outline: 'none', marginBottom: '20px', boxSizing: 'border-box' }} />
            <h2 style={{ color: '#FFF', margin: '0 0 12px 0', fontSize: '22px' }}>Full Birth Name (Optional):</h2>
            <input type="text" value={otherFullName} onChange={(e) => setOtherFullName(e.target.value)} placeholder="For Destiny numbers..." style={{ width: '100%', backgroundColor: '#222', color: '#FFF', fontSize: '24px', padding: '16px', borderRadius: '12px', border: '2px solid #555', outline: 'none', marginBottom: '24px', boxSizing: 'border-box' }} />
            <button onClick={() => setOtherNumbers(calculateAll(otherBday, otherFullName))} style={{ width: '100%', backgroundColor: '#FF9500', color: '#000', fontSize: '24px', fontWeight: 'bold', border: 'none', borderRadius: '16px', padding: '20px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px' }}><Calculator size={28} /> Calculate Blueprint</button>
          </div>
          {otherNumbers && renderReadingCard(otherNumbers, otherName || "Friend", () => saveReadingToHistory('other'))}
        </>
      )}

      {view === 'history' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {history.length === 0 ? <p style={{ color: '#888', textAlign: 'center', fontSize: '20px' }}>No saved readings.</p> : history.map(entry => (
            <div key={entry.id} style={{ backgroundColor: '#222', border: '2px solid #444', borderRadius: '16px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <div><div style={{ color: '#FFF', fontSize: '24px', fontWeight: 'bold' }}>{entry.name}</div><div style={{ color: '#888', fontSize: '18px' }}>{entry.savedAt}</div></div>
                <div style={{ backgroundColor: '#FF9500', color: '#000', fontSize: '28px', fontWeight: 'bold', width: '48px', height: '48px', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{entry.numbers.lifePath}</div>
              </div>
              {entry.notes && <div style={{ backgroundColor: '#111', padding: '12px', borderRadius: '12px', marginBottom: '16px', borderLeft: '4px solid #3B82F6' }}><div style={{ fontSize: '16px', color: '#CCC', fontStyle: 'italic' }}>"{entry.notes}"</div></div>}
              <div style={{ display: 'flex', gap: '12px' }}>
                <button onClick={() => exportReading(entry.name, entry.birthdate, entry.numbers, entry.notes)} style={{ flex: 1, backgroundColor: '#112244', border: '2px solid #3B82F6', color: '#3B82F6', padding: '12px', borderRadius: '12px', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}><Share2 size={20} /> Export</button>
                <button onClick={() => deleteReading(entry.id)} style={{ backgroundColor: 'transparent', border: '2px solid #FF3B30', color: '#FF3B30', padding: '12px 20px', borderRadius: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Trash2 size={24} /></button>
              </div>
            </div>
          ))}
        </div>
